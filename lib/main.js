...
var getWidgetContent = function (s, t) {
  ...
  html += ('<img src="' + data.url(iconUrl) + '" style="display:inline-block" />');
...
myPanel.port.on('form-timer', function (form) {
  ...
  myWidget.width = widgetWidthAccordingToTime(form.timeHumanized);
  myWidget.content = getWidgetContent(form.time, settings.timerHumanized);
  ...
  settings.currentIntervalID = require("timers").setInterval(function() {
    ...
    if (i === 0) {
      ...
      var time = getHumanizedDate(secondsToSlices(i));
